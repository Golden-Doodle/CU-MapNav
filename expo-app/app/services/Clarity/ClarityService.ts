...
export async function sendCustomEventToClarity(
  event: string
): Promise<boolean> {
  try {
    if (event == null || event.length < 1) {
      throw new Error(
        "An empty string is an invalid event for usability testing"
      );
    }
...
export function toggleClarity(
  ...
  clarityIsOn ? runClarity(config) : pauseClarity();
}
